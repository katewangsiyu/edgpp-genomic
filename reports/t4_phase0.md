...
## 6. CADD+Borzoi regime 实验（Day 9 补充）
...
| CADD+Borzoi | Complex | **0.3508** | 0.3348 | 0.7216 |

### CADD+Borzoi 上的 Selective Head

| Regime | Dataset | λ_cv | Spearman | cov@10% Δ | cov@25% Δ | cov@50% Δ |
|---|---|---|---|---|---|---|
| Borzoi-only | Mendelian | 1.0 | 0.391 | +0.007 | +0.050 | **+0.056** |
| Borzoi-only | Complex | 2.0 | 0.221 | +0.010 | +0.007 | −0.006 |
| **CADD+Borzoi** | **Mendelian** | **0.0** | 0.229 | **0.000** | **0.000** | **0.000** |
| **CADD+Borzoi** | **Complex** | 1.0 | 0.299 | +0.008 | +0.001 | −0.004 |

**关键发现**: CADD+Borzoi Mendelian 上 λ_cv=0 — 内层 CV 判定 reliability 完全无用。原因：GBM 的 7 个特征 (GC, CpG, priPhyloP, mamPhyloP, verPhyloP, bStatistic, tss_dist) 与 CADD 114 列高度重叠，LogReg 已经利用了这些信息。

**影响**: selective head 本质是 conservation feature engineering 层。仅在 base model 未见过这些特征时有效（Borzoi-only），在 supervised SOTA (CADD+Borzoi) 上完全冗余。

---

## 7. 修订后的论文路径评估

### 原 Path B (selective head paper) → **已关闭**
CADD+Borzoi regime 实验证明方法在竞争性 baseline 上无效，不足以独立成文。

### Path A: Coverage-AUPRC benchmark paper (仍可行但需调整)
- Selective head 不能作为贡献核心，改为"case study showing feature-overlap limitation"
- 主贡献改为：系统评测 coverage-AUPRC 在所有 TraitGym 方法上的表现
- 投稿：Bioinformatics application note / RECOMB poster

### Path C: Falsification story (推荐，材料最充分)
- Day 5 side-feature 泄漏 + Day 9 特征冗余 = 两个互补的 cautionary tales
- "Reliability estimators in genomic variant scoring: when does confidence help?"
- 投稿：ICML GenBio Workshop / ML4H / MLCB

### Path D: Conformal prediction pivot (高风险高回报)
- 如果能证明 conformal guarantees 在 chrom-LOO 下成立 → 理论贡献
- 投稿：ICLR 2027
- 需要全新开发，5 个月时间紧张

### 新 Path E: 非 CADD 特征挖掘
- 找到 CADD 不包含的、对 LogReg 残差有预测力的特征
- 例如：3D genome structure, epigenetic marks, regulatory annotations
- 需要新数据源，工作量未知
...
### 需要决策
1. **论文路径选择**: A / C / D / E（B 已关闭）
2. **时间线**: RECOMB 2027 (~2026-11) 给了 ~7 个月；ICLR 2027 (~2026-09) 给了 ~5 个月
3. **5090 申请**: Path A/C 不需要，Path D/E 待定
