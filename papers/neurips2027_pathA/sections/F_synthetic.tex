...
\caption{Synthetic $n$-sweep validating the $O(n^{-1/2})$ rate of Theorem~\ref{thm:t5oracle}. \textbf{(a) Log-log empirical rate.} Worst-cell gap $G(\hat K_{\mathrm{CV}})$ vs $n$ on log-log axes; six curves cover $d \in \{8, 32, 128\}$ (color) $\times$ $\pi_{\min} \in \{0.1, 0.5\}$ (linestyle); $\pm 1$ SE bars over 5 replicates. Black dashed lines: $G = \bar C(\pi_{\min}) / \sqrt{n}$ with empirical constants from panel (b), so the dashed reference is purely descriptive of the compensated mean. The empirical fit on $n \geq 1{,}000$ across all 6 curves gives slope $-0.44 \pm 0.08$ (mean $\pm$ std); T5.1 predicts $-0.50$, within $1$\,SD. The dimension-free claim is visible as overlapping $d$-curves at matched $\pi_{\min}$. \textbf{(b) Compensated plot.} $G(\hat K_{\mathrm{CV}}) \cdot \sqrt{n}$ vs $n$ on log-x linear-y axes: under T5.1's $O(n^{-1/2})$ rate this product should be approximately constant in $n$. Red dashed lines mark empirical means $\bar C(\pi_{\min})$ over $n \geq 1{,}000$; their ratio $1.62$ is below the T5.1 asymptotic upper-bound ratio $\sqrt{5} \approx 2.24$, consistent with T5.1 (which bounds $G$ from above). Gray bands flag the $n < 1{,}000$ small-sample fallback regime excluded from the fit.}
\label{fig:n_sweep}
\end{figure}
