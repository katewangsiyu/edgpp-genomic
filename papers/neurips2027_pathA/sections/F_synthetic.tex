...
\centering
\small
\caption{Worst-cell coverage gap $G(\hat K_{\mathrm{CV}})$ (mean $\pm$ SE over 5 replicates) on synthetic data, $L_F = 4$. The dimension-free claim of T5.1 is visible as approximately matching values across $d$ at fixed $(n, \pi_{\min})$; the $\pi_{\min} = 0.50$ rows are smaller than $\pi_{\min} = 0.10$ at matched $n$, with empirical compensated-constant ratio $\bar C_{0.10} / \bar C_{0.50} = 1.62$ vs the T5.1 asymptotic upper bound $\sqrt{5} \approx 2.24$ (T5.1 bounds $G$ from above, so empirical ratios below $\sqrt{5}$ are consistent with the bound).}
...
The log-log slope of $G(\hat K_{\mathrm{CV}})$ versus $n$, fit per-curve across $n \geq 1{,}000$ to avoid the small-sample fallback regime, is $-0.44 \pm 0.08$ (mean $\pm$ std over the 6 fits across $d \in \{8, 32, 128\}$ and $\pi_{\min} \in \{0.1, 0.5\}$); the T5.1 prediction of $-0.50$ lies within $1$ standard deviation of the empirical mean, consistent with the predicted rate. Figure~\ref{fig:n_sweep} visualises the result. Figure~\ref{fig:n_sweep}(b) additionally compensates for the rate by plotting $G(n) \cdot \sqrt{n}$, which T5.1 predicts to be approximately constant in $n$; the empirical constants are $\bar C_{0.10} = 3.00$ and $\bar C_{0.50} = 1.86$, with ratio $1.62$ versus the T5.1 asymptotic upper bound ratio $\sqrt{5} \approx 2.24$ (T5.1 is an upper bound, so empirical ratios $\leq \sqrt{5}$ are consistent).

\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{figures/fig_n_sweep.pdf}
\caption{Synthetic $n$-sweep validating the $O(n^{-1/2})$ rate of Theorem~\ref{thm:t5oracle}. Worst-cell gap $G(\hat K_{\mathrm{CV}})$ vs calibration size $n$ on log-log axes, split by $\pi_{\min}$ (left $0.10$, right $0.50$); three coloured curves per panel cover feature dimensions $d \in \{8, 32, 128\}$ with $\pm 1$ SE bars over 5 replicates. Y-axes are shared, so the $\pi_{\min}^{-1/2}$ vertical offset is visible at a glance: left-panel curves sit roughly $\bar C_{0.10}/\bar C_{0.50} = 1.62 \times$ above the right-panel curves at matched $n$, below the T5.1 asymptotic upper-bound ratio $\sqrt{5} \approx 2.24$ (T5.1 is an upper bound on $G$, so empirical ratios below $\sqrt{5}$ are consistent with the bound). Black dashed reference: $G = \bar C(\pi_{\min}) / \sqrt{n}$ where $\bar C(\pi_{\min})$ is the empirical compensated mean $G(n)\cdot\sqrt{n}$ averaged over $n \geq 1{,}000$ and $d$ within each panel; this descriptive reference shows where the empirical $-1/2$ slope passes. Per-panel slope fits annotated in upper-right boxes; cross-panel summary in the title gives slope $-0.44 \pm 0.08$ (mean $\pm$ std over 6 fits), with T5.1 prediction $-0.50$ within $1$ standard deviation. Gray bands flag the $n < 1{,}000$ small-sample fallback regime excluded from the fit. The dimension-free claim is visible as overlapping $d$-curves within each panel.}
\label{fig:n_sweep}
\end{figure}
