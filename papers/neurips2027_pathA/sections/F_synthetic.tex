...
The log-log slope of $G(\hat K_{\mathrm{CV}})$ versus $n$, fit across $n \geq 1{,}000$ to avoid the small-sample fallback regime, is $-0.50 \pm 0.05$ (averaged over $d \in \{8, 32, 128\}$ and $\pi_{\min} \in \{0.1, 0.5\}$), within bootstrap CI of the predicted $-1/2$. Figure~\ref{fig:n_sweep} (§\ref{sec:experiments}) visualises the result.
