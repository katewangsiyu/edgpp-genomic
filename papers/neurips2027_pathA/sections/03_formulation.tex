...
\begin{assumption}[A1$'$ chrom-wise i.i.d.\ (strengthening of A1)]
\label{ass:A1prime}
$\{(X_i, Y_i) : C_i = c\} \overset{i.i.d.}{\sim} P_c$. T3 and T5 use A1$'$ for the cleaner sampling argument; T3$'$ uses only A1 for robustness.
\end{assumption}
...
Assumption~\ref{ass:ASL} is used only by T5 (§\ref{sec:theory:t5}) to derive the optimal bin count $K^\star$. Empirical audit of Assumption~\ref{ass:ASL} via per-pair Lipschitz ratios on $\shat$-bin pairs (App.~\ref{app:data}, Fig.~\ref{fig:asl_audit}) gives noise-adjusted upper envelope $L_F^{(95\%)} = 13.8$ (Mendelian) and $8.0$ (Complex); LCLS estimator \citep{lcls2024lipschitz} gives a tighter typical-slope estimate $\hat L_F = 2.97$ / $4.51$.
