...
\centering
\small
\caption{Head-to-head against off-the-shelf normalized + Mondrian CP, sharing the same $(\phat, \shat)$. Partition $K$ for B2 / B4 selected per outer fold via proper nested chrom-LOO with $K_{\mathrm{eval}}$-fair scoring (Mendelian $K_{\mathrm{eval}} = 3$, Complex $K_{\mathrm{eval}} = 5$). Single-seed point estimates. \textbf{Of the four variants we evaluated, HCCP is the only frontier point at this corner on both datasets.}}
\label{tab:h2h}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{llcccc}
\toprule
Dataset & Method & Marg.\ cov.\ & Cov$|Y{=}1$ & $\shat$-bin gap $\downarrow$ & Per-chrom gap \\
\midrule
