...
\item {\bf Open access to data and code}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: TraitGym matched\_9, ProteinGym, and Open Targets Platform 25.06 are publicly distributed by their creators (the Open Targets bulk parquets are mirrored on EBI FTP at \texttt{ftp.ebi.ac.uk/pub/databases/opentargets/platform/25.06/output/}). Anonymized code (aggregator training, HCCP calibration, nested-CV $K$-selection, bootstrap) is uploaded as a supplementary archive (\texttt{supplementary.zip}) accompanying this submission, with end-to-end pipeline scripts inline-referenced from the main text: \texttt{T\_tools/cp\_baselines\_h2h.py} for the H2H sweep of Tab.~\ref{tab:keval_sensitivity}, \texttt{T\_tools/trait\_cluster\_aggregate.py} for the trait-clinical clustering of Tab.~\ref{tab:trait_cluster}, \texttt{T\_tools/synthetic\_n\_sweep.py} for the synthetic rate validation of App.~\ref{app:synthetic}, and \texttt{scripts/40--44\_open\_targets\_*.py} + \texttt{T\_tools/open\_targets\_subsample.py} for the cross-platform Open Targets replication of App.~\ref{app:open_targets}. Permissive-license public release on acceptance.
...
\item {\bf Declaration of LLM usage}
    ...
    \item[] Answer: \answerNA{}
    \item[] Justification: LLMs were not used as any component of the core methodology. The aggregator $\phat$ is a gradient-boosted classifier and the variance head $\shat$ is a gradient-boosted regressor; both consume pre-computed numeric features (CADD, Borzoi, GPN-MSA scores) rather than language.
