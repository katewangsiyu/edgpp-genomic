...
\item {\bf Open access to data and code}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: TraitGym matched\_9 and ProteinGym are publicly distributed by their creators. Code (aggregator training, HCCP calibration, nested-CV $K$-selection, bootstrap) is provided as anonymized supplementary material at submission time and will be released under a permissive licence on acceptance; pipeline scripts (\texttt{T\_tools/cp\_baselines\_h2h.py} for the H2H sweep referenced in Tab.~\ref{tab:keval_sensitivity}, \texttt{T\_tools/trait\_cluster\_aggregate.py} for the trait-clinical clustering of Tab.~\ref{tab:trait_cluster}, \texttt{T\_tools/synthetic\_n\_sweep.py} for the synthetic rate validation of App.~\ref{app:synthetic}) are referenced inline.
...
\item {\bf Licenses for existing assets}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: TraitGym \citep{benegas2025traitgym}, ProteinGym \citep{notin2023proteingym}, CADD \citep{kircher2014cadd}, Borzoi \citep{linder2025borzoi}, GPN-MSA \citep{benegas2023gpnmsa}, AlphaGenome \citep{avsec2026alphagenome} are all cited via the bibliography with full publication metadata; the \texttt{crepes} package \citep{bostrom2022crepes} is used for the H2H baselines and acknowledged. We use each asset in accordance with its publicly distributed licence (TraitGym: MIT; ProteinGym: CC-BY-4.0; CADD: free academic; Borzoi/GPN-MSA: open-source per the original repos; AlphaGenome: Google API terms; \texttt{crepes}: BSD-3).
